    ...
    getSource(id, url) {
        ...
        var callback = function(response) {
            ...
            response.on('end', function () {
                ...
                var sourceString = responseString;
                var sourceSize = responseString.length;
                var sourceLimit = 10000;
                var sourceArray = [];
                var sourceCounter;

                for (sourceCounter = 0; sourceCounter < sourceSize; sourceCounter += sourceLimit) {
                    sourceArray.push(sourceString.substr(sourceCounter, sourceLimit));
                }
            ...
            response.on('error', function(e) {
                console.log('[%d] SOURCE ERROR: (%s)', Date.now(),
                    this.getUser(id), e.message);
            });
